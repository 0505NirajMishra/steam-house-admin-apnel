...
class CompanyListController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $meter = DB::table('users')->where('meter_id', $input['meter_id'])->get();

        if (count($meter) > 0) {
            return redirect()->back()->with('success', 'Meter Id Allready Exist');
        }
    ...
    public function update(Request $request, User $user)
    {
        ...
        if(!empty($input['meter_id'])){

            $meter = DB::table('users')->where('id',$id)->where('meter_id', $input['meter_id'])->first();
            if ($meter) {
                $input['meter_id']=$request->meter_id;
                // return redirect()->back()->with('success', 'Meter Id Allready Exist');
            }else{
                $meterdata = DB::table('users')->where('meter_id', $input['meter_id'])->get();
                if(count($meterdata)>0){
                    return redirect()->back()->with('warning', 'Meter Id Allready Exist');
                }
