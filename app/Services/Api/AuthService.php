...
class AuthService
{
     ...
     * @return \Illuminate\Http\Response
     */
    public static function companyRegister(Request $request)
    {
        ...
        //             'status' => false,
        //             // 'message' => 'You have an account, Please login with same credentials.',
        //             'message' => 'Meter Id Already Exist.',
        //             // 'type' => 'unauthorized',
        //         ],
        //         200
        //     );
        ...
        if($user){
            ...
            if (!empty($input['image'])) {
                $image = FileService::ImageUploader($request, 'image', 'company/image/');
                $input['image'] = json_encode($image);
            }
...
/**
    ...
    public static function deleteUserAccount(Request $request){  
        $result=UserService::delete_companyapi($request);
           if ($result) {
               return response()->json(
                   [
                       'status' => true,
...
//Services Request
    ...
    public static function servicesrequestapi(Request $request)
    {
        ...
        $detail1['user_id']=auth()->user()->id;
        $detail1['date']=$request->date;
        $detail1['time']=$request->time;
        
        $detail1['body']='New Service Request';
      
        UserService::sendsteammail($detail1,$newmail);
