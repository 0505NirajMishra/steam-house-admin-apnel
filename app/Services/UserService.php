...
// 
/**
     * Send Booking Mail On Tawasul Mail
     *
     * @param Int $id
     * @param Array $relations
     
     */
    public static function sendsteammail(array $detail, $request)
    {
       
       try{

        $data=DB::table('users')->select('*')->where('id',$detail['user_id'])->first();
        ...
        $data=Mail::to($request)->send(new \App\Mail\SendEmail($details));
    ...
    public static function delete_companyapi($request){
        $result=DB::table('users')->where('id',auth()->user()->id)->delete();
        $result1=DB::table('service_request')->where('user_id',auth()->user()->id)->delete();
        return $result.$result1;
    }
